...
export default function Home() {
  ...
  return (
    <>    
      ...
      <div className='w-full max-w-[2560px] m-auto flex md:flex-row flex-col-reverse justify-between relative top-[10rem] gap-5'>  
      ...
      <div className='relative top-[10rem] py-20 md:gap-7 gap-y-20 parent md:w-full w-[80%] m-auto'>
          <motion.div
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          transition={{ duration: 1 }}
          className='text-center flex flex-col items-center'><h1 className='md:text-5xl text-4xl font-bold text-[var(--orange)]'>350+</h1><p className='pt-3 md:text-2xl text-lg text-center tracking-wider'>Successfull projects</p></motion.div>
          <motion.div
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          transition={{ duration: 1 }} 
          className='text-center  flex flex-col items-center'><h1 className='md:text-5xl text-4xl font-bold text-[var(--orange)]'>50+</h1><p className='pt-3 md:text-2xl text-lg text-center tracking-wider'>Team members</p></motion.div>
          <motion.div
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          transition={{ duration: 1 }}
           className='text-center flex flex-col items-center'><h1 className='md:text-5xl text-4xl font-bold text-[var(--orange)]'>100+</h1><p className='pt-3 md:text-2xl text-lg text-center tracking-wider'>Happy clients</p></motion.div>
          <motion.div
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          transition={{ duration: 1 }} className='text-center flex flex-col items-center'><h1 className='md:text-5xl text-4xl font-bold text-[var(--orange)]'>100%</h1><p className='pt-3 md:text-2xl text-lg text-center tracking-wider'>Client satisfaction</p></motion.div>
      </div>

      <div className='relative top-[5rem] py-20 gap-y-10 parentTwo md:w-[90%] w-[80%] m-auto'>
        <div className=' rounded-full py-5 px-7 flex flow-row border border-[var(--light-grey)] items-center '>
          <div className=' rounded-full bg-[var(--light-grey)] w-8 h-8 flex justify-center items-center'><img  className='w-4' src="https://res.cloudinary.com/dcraqvlmb/image/upload/f_auto,q_auto/uv7gqqnre6rlfjtzqc8w" alt="done icone" /></div>
           <h1 className='pl-5 text-xl'>Project Portfolio <br /> Showcase</h1>
        </div>
        <div className='rounded-full py-5 px-7 flex flow-row border border-[var(--light-grey)] items-center '>
          <div className=' rounded-full bg-[var(--light-grey)] w-8 h-8 flex justify-center items-center'><img  className='w-4' src="https://res.cloudinary.com/dcraqvlmb/image/upload/f_auto,q_auto/uv7gqqnre6rlfjtzqc8w" alt="done icone" /></div>
           <h1 className='pl-5 text-xl'>Project Portfolio <br /> Showcase</h1>
